                    ...
                    <div class="card-body pt-0">
                        ...
                        <div class="row col-lg-12">
                            <div class="form-group col-xl-5">
                              ...
                              <input type="file" class="form-control-file" name="file_stpm" id="file_stpm" placeholder="" aria-describedby="fileHelpId">
                              <small id="fileHelpId" class="form-text text-muted">pdf, png, jpeg</small>
                            </div>
                            @if ($stpmm->dokumen != '')
                                <div class="col-xl-5">
                                    <a class="btn btn-info" target="_blank" href="{{ url('storage/'.$stpmm->dokumen) }}" role="button">Papar Sijil</a>
                                </div>
                            @endif
                        </div>
                    </div>
                                ...
                                @if (count($k_stpm)>0)
                                    @foreach ($k_stpm as $kstpm)
                                        <tr class="align-middle">
                                            ...
                                            <td>
                                                <input type="hidden" name="addMoreInputFields[{{ $i }}][id_keputusan]" id="id_keputusan" value="{{ $kstpm->id }}">
                                                <select class="form-control select2bs4" name="addMoreInputFields[{{ $i }}][matapelajaran]" required>
                                                    <option value="">Sila Pilih</option>
                                                    @foreach ($mtstpm as $mstpm)
                                                        <option value="{{ $mstpm->id }}" {{ $mstpm->id == $kstpm->matapelajaran? 'selected' : '' }}>{{ $mstpm->subjek }}</option>
                                                    @endforeach
                                                </select>
                                            </td>
                                            <td class="text-center">
                                                <select class="form-control select2bs4" name="addMoreInputFields[{{ $i }}][gred]" required>
                                                    ...
                                                    @foreach ($gredstpm as $gred)
                                                        <option value="{{ $gred->gred }}" {{ $gred->gred == $kstpm->gred ? 'selected' : '' }}>{{ $gred->gred }}</option>
                                                    @endforeach
                                                </select>
                                            </td>
                                            <td class="text-center ">
